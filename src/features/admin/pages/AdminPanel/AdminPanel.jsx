...
import { useCategoriesContext } from '../../../products/context/CategoriesContext';
...
const AdminPanel = () => {
  ...
  const generateKeywords = (name) => {
    ...
    const commonSuffixes = ['', 's', 'es', 'a', 'o'];
    const generatedKeywords = new Set();

    nameKeywords.forEach(word => {
      commonSuffixes.forEach(suffix => {
        generatedKeywords.add(word + suffix);
        generatedKeywords.add(word.replace(/[^a-z0-9]/g, '') + suffix);
      });
    });

    for (let i = 0; i < nameKeywords.length - 1; i++) {
      const combo = `${nameKeywords[i]}${nameKeywords[i + 1]}`;
      generatedKeywords.add(combo);
    }
  ...
  const handleImageChange = async (e) => {
    ...
    if (!file.type.match('image.*')) {
      setMessage({ text: 'Solo se permiten imágenes', type: 'error' });
      return;
    }
    ...
    try {
      ...
      setMessage({
        text: 'Error al subir: ' + (error.message || 'Intenta con otra imagen'),
        type: 'error'
      });
    } finally {
      setUploading(false);
  ...
  const handleSubmit = async (e) => {
    ...
    if (product.salesCount < 0) errors.push('El contador de ventas no puede ser negativo');
